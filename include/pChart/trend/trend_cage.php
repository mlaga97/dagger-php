...
//print_r($_SESSION);
 ...
 $query = "SELECT id, date, cage_1+cage_2+cage_3+cage_4 as cage_score, cage_1, cage_2, cage_3, cage_4 FROM msihdp.response where pt_id = '". $info_store ."'and cage_check = 1 and clinic_id in (select clinic_id from groups where user_id = '" . $id_store ."')";
 //echo $query;	
...
if($result = $mysqli->query($query)){
	if($result->num_rows > 1 ){
		while($row = $result->fetch_assoc())  {
			...
			//echo " ". $value;
			if ($row['cage_1'] >= 0 && $row['cage_2'] >= 0 &&
				$row['cage_3'] >= 0 && $row['cage_4'] >= 0 ){
					$scores[$row['id']] = $row['cage_score'];
...
/* Create and populate the pData object */
	...
	$myPicture->drawText(10,16,"Trend of CAGE Scores for Client ID " . $id_store, array("FontSize"=>11,"Align"=>TEXT_ALIGN_BOTTOMLEFT));
	...
	$myPicture->setGraphArea(50,30,$width,$vHeight);
 	...
 	/* Add cutoff data */
	$myPicture->drawThreshold(2,array("Alpha"=>255,"Ticks"=>2,"R"=>255,"G"=>0,"B"=>0, "WriteCaption"=>true, "Caption"=>Significant, "CaptionR"=>255, "CaptionB"=>255,"CaptionG"=>255, "CaptionOffset"=>TRUE));
