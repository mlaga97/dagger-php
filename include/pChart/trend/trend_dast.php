...
//print_r($_SESSION);
 ...
 $query = "SELECT id, date, dast_1+dast_2+dast_3+dast_4+dast_5+dast_6+dast_7+dast_8+dast_9+dast_10 as dast_score, dast_1, dast_2, dast_3, dast_4, dast_5, dast_6, dast_7 , dast_8, dast_9, dast_10 FROM msihdp.response where pt_id = '". $info_store ."'and ces_check = 1 and clinic_id in (select clinic_id from groups where user_id = '" . $id_store ."')";
 //echo $query;	
...
if($result = $mysqli->query($query)){
	if($result->num_rows > 1 ){
		while($row = $result->fetch_assoc())  {
			...
			//echo " ". $value;
			if (
				$row['dast_1']  >= 0 	&& $row['dast_2'] >= 0  &&
				$row['dast_3']  >= 0 	&& $row['dast_4'] >= 0  &&
				$row['dast_5']  >= 0 	&& $row['dast_6'] >= 0  &&
				$row['dast_7']  >= 0 	&& $row['dast_8'] >= 0  &&
				$row['dast_9']  >= 0 	&& $row['dast_10'] >= 0 &&
				$row['dast_1']  != "" 	&& $row['dast_2'] != "" &&
				$row['dast_3']  != "" 	&& $row['dast_4'] != "" &&
				$row['dast_5']  != "" 	&& $row['dast_6'] != "" &&
				$row['dast_7']  != "" 	&& $row['dast_8'] != "" &&
				$row['dast_9']  != "" 	&& $row['dast_10']!= ""){
					$scores[$row['id']] = $row['dast_score'];
...
/* Create and populate the pData object */
	...
	$myPicture->drawText(10,16,"Trend of DAST-10 Scores for Client ID " . $id_store, array("FontSize"=>11,"Align"=>TEXT_ALIGN_BOTTOMLEFT));

	 /* Draw the scale */
	$AxisBoundaries = array(0=>array("Min"=>0,"Max"=>11));
	$myPicture->setGraphArea(50,30,$width,$vHeight);
 	...
 	/* Add cutoff data */
 	$myPicture->drawThreshold(10,array("Alpha"=>255,"Ticks"=>2,"R"=>0,"G"=>0,"B"=>0, "WriteCaption"=>true, "Caption"=>Maximum, "CaptionR"=>255, "CaptionB"=>255,"CaptionG"=>255, "CaptionOffset"=>TRUE));
	$myPicture->drawThreshold(9,array("Alpha"=>255,"Ticks"=>2,"R"=>255,"G"=>0,"B"=>0, "WriteCaption"=>true, "Caption"=>Severe, "CaptionR"=>255, "CaptionB"=>255,"CaptionG"=>255, "CaptionOffset"=>TRUE));
	$myPicture->drawThreshold(6,array("Alpha"=>255,"Ticks"=>2,"R"=>0,"G"=>0,"B"=>255, "WriteCaption"=>true, "Caption"=>Substantial ));
	$myPicture->drawThreshold(3,array("Alpha"=>255,"Ticks"=>2,"R"=>0,"G"=>0,"B"=>255, "WriteCaption"=>true, "Caption"=>Moderate, ));
	$myPicture->drawThreshold(1,array("Alpha"=>255,"Ticks"=>2,"R"=>0,"G"=>255,"B"=>0, "WriteCaption"=>true, "Caption"=>Low, ));
