...
//print_r($_SESSION);
 ...
 $query = "SELECT id, date, gad_1+gad_2+gad_3+gad_4+gad_5+gad_6+gad_7 as gad_score, gad_1, gad_2, gad_3, gad_4, gad_5, gad_6, gad_7 FROM msihdp.response where pt_id = '". $info_store ."'and gad_check = 1 and clinic_id in (select clinic_id from groups where user_id = '" . $id_store ."')";
 //echo $query;	
...
if($result = $mysqli->query($query)){
	if($result->num_rows > 1 ){
		while($row = $result->fetch_assoc())  {
			...
			//echo " ". $value;
			if ($row['gad_1'] >= 0 && $row['gad_2'] >= 0 &&
				$row['gad_3'] >= 0 && $row['gad_4'] >= 0 &&
				$row['gad_5'] >= 0 && $row['gad_6'] >= 0 &&
				$row['gad_7'] >= 0){
					$scores[$row['id']] = $row['gad_score'];
...
/* Create and populate the pData object */
	...
	$myPicture->drawText(10,16,"Trend of GAD-7 Scores for Client ID " . $id_store, array("FontSize"=>11,"Align"=>TEXT_ALIGN_BOTTOMLEFT));

	 /* Draw the scale */
	$AxisBoundaries = array(0=>array("Min"=>0,"Max"=>21));
	...
	$myPicture->drawThreshold(15,array("Alpha"=>255,"Ticks"=>2,"R"=>255,"G"=>0,"B"=>0, "WriteCaption"=>true, "Caption"=>Severe));
	$myPicture->drawThreshold(21,array("Alpha"=>255,"Ticks"=>2,"R"=>0,"G"=>0,"B"=>0, "WriteCaption"=>true, "Caption"=>Maximum));
