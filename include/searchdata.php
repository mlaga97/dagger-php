          ...
          if($_SESSION['admin'] == 1)
          {
            ...
            echo " patients so far!<br><br>";
                 ...
                 if(array_key_exists('end_date',       $_POST)  &&
                  ...
                  array_key_exists('employee_select',  $_POST)  &&
                  $_POST['start_date']      != ""               &&
                  $_POST['end_date']        != ""               )
                 {
                 ...
                 if(array_key_exists('end_date',        $_POST)  &&
                  array_key_exists('start_date',        $_POST)  &&
                  array_key_exists('clinic_select',     $_POST)  &&
                  $_POST['start_date']      != ""                &&
                  $_POST['end_date']        != ""                )
                  {
                    $startdate    = $_POST['start_date'];
...
//            echo "<link rel=\"stylesheet\" href=\"/resources/demos/style.css\" />";
          ...
          ?> 
           <hr><br><h4>Please enter the patient ID here to search by.</h4>
...
else
{
 ...
 /* I omitted this portion since it was not working.
  
          //The following code will allow the users to be set as active or inactive by an administrator. 
          echo "<form id=\"form3\"  method=\"post\" > ";
          echo "<br><br><b>Set a user to active or inactive. </b>";
          $mysqli = new mysqli(DB_SERVER, DB_USER, DB_Password, DB_NAME);
          ...
          if($_SESSION['admin'] == 1)
          {
              ...
              $employee = "SELECT id,name,active from users where id in (select user_id from groups where clinic_id in(select clinic_id from groups where user_id ='$user_id_variable'))";
              $employee_info = $mysqli->query($employee);
              $employee_select = $employee_info->fetch_assoc();


              // Here we begin printing our table. This table will contain the buttons for selecting employees as active or inactive.
              echo "<table width=\"640\"><td><h4><b> Employee </b></h4></td>    <td><h4><b> Active </b></h4></td>    <td><h4><b> Inactive </b></h4></td>\n";
              $i = 0;
              //$checked = "";
              while($employee_select)
              { 
              echo "<tr>";  
                if($employee_select)
                {
                  if ($employee_select['active'] == '1')
                  {
                    $active = "checked";
                    $inactive = "";
                  }
                  else if($employee_select['active'] == '0')
                  {
                    $active = "";
                    $inactive = "checked";
                  }
                  ...
                  //<name =\"employee_select\" value=\"".$employee_select['id']."\"/><td> Took this out.
                  echo "<td style= 'text-align: left'>".$employee_select['name']."</td><td><input type=\"radio\"  width=\"260\"value=\"1\"  $active name =\"active".$i."\"></td><td><input type=\"radio\"  width=\"260\" value=\"2\"  $inactive name =\"active".$i."\"></td>\n";
              ...
              echo "</td></table><br>";
              echo "<br><br><button onclick =\"update_employee_activity(form3, $i);\"  type = \"submit\" style= \"height: 40px; width: 100px\">Change active users</button>";
              echo "</form>";
              }
