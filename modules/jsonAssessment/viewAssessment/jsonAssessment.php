...
function showScore($assessment, $scoreType) {
	...
	switch($scoreType) {
		...
		case "averageValue_excludingBlank":
			...
			echo "<table><tr><td>" . $category . "</td><td class='score'>" . round(($total/$count), $rnd_precision) . "</td></tr></table>";
			break;
...
foreach($assessments as $assessment) {
	if($_SESSION[$assessment["metadata"]["id"]]) {
		echo "<div id='" . $assessment["metadata"]["id"] . "_viewAssessment_container' class='jsonAssessment'>";
				...
				case "human_readable":
					if(array_key_exists("questions", $assessment)) {
						...
						foreach($assessment["questions"] as $question) {
							...
							if(array_key_exists($question["id"], $_SESSION)) {
								if($_SESSION[$question["id"]] == -1) {
									echo "<tr><td>" . $question["text"] . "</td><td class='response'>" . "No Response" . "</td></tr>";
								} else {
									echo "<tr><td>" . $question["text"] . "</td><td class='response'>" . array_search($_SESSION[$question["id"]], $assessment["types"][$question["type"]]["options"]) . "</td></tr>";
								}
							} else {
								echo "<tr><td>" . $question["text"] . "</td><td class='response'>" . "No Response" . "</td></tr>";
							}
					...
					if(array_key_exists("sections", $assessment)) {
						...
						foreach($assessment["sections"] as $section) {
							foreach($section["questions"] as $question) {
								...
								if(array_key_exists($question["id"], $_SESSION)) {
									if($_SESSION[$question["id"]] == -1) {
										echo "<tr><td><ol start='" . substr($question["id"], strpos($question["id"], "_") + 1) . "'><li>" . $question["text"] . "</li></ol></td><td class='score'>" . "No Response" . "</td></tr>";
									} else {
										echo "<tr><td><ol start='" . substr($question["id"], strpos($question["id"], "_") + 1) . "'><li>" . $question["text"] . "</li></ol></td><td class='score'>" . array_search($_SESSION[$question["id"]], $assessment["types"][$question["type"]]["options"]) . "</td></tr>";
									}
								} else {
									echo "<tr><td><ol start='" . substr($question["id"], strpos($question["id"], "_") + 1) . "'><li>" . $question["text"] . "</li></ol></td><td class='score'>" . "No Response" . "</td></tr>";
								}
		...
		echo "</div><!-- END " . $assessment["metadata"]["id"] . "_viewAssessment_container-->";
	}
}
?>
